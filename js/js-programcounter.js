...
/**
 * initialises and sets up display and UI events for the program counter
 */
function initialise_program_counter()
{
   const pc_leds = $(".program-counter-led");
   const pc_ce = $("#control-PC-CE");
   const pc_ci = $("#control-PC-PI");
   const pc_co = $("#control-PC-PO");
   program_counter = new VirtualCounter({
      clock: clock,
      data_bus: data_bus,
      control_bus: control_bus,
      counter_bits: register_size,
      read_data_index: PC_PI,
      write_data_index: PC_PO,
      count_enable_index: PC_CE
   });
   pc_ce.click(() => {
      control_bus.set(PC_CE);
      pc_ce.find(".led").toggleClass("led-on");
   });
   pc_ci.click(() => {
      control_bus.set(PC_PI);
      pc_ci.find(".led").toggleClass("led-on");
   });
   pc_co.click(() => {
      control_bus.set(PC_PO);
      pc_co.find(".led").toggleClass("led-on");
   });
   clock.add_falling_client(() => {
      for(let i = 0; i < register_size; ++i)
      {
         if(program_counter.get(register_size - i - 1))
            $(pc_leds[i]).addClass("led-on");
         else
            $(pc_leds[i]).removeClass("led-on");
      }
   });
}
